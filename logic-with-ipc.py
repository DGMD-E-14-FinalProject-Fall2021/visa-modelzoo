...
class Scanner:
        def __init__(self, id, loop):
            ...
            self.submit_async(self.run_ble_consumer(queue), loop)
            self.submit_async(run_ble_client(queue), loop)
            
            self.raw_data = self.run_ble_consumer.data
            self.event = Event()
            self.data_lock = Lock()
        ...
        async def run_ble_consumer(self, queue: asyncio.Queue):
            while True:
                with self.data_lock:
                ...
                # Not sure you need this wait. Play with it.
...
scanner = Scanner(1, _loop)

#for frame1 in camera.capture_continuous(rawCapture, format="bgr",use_video_port=True):
while True:

        # get distance data from the BLE client here
        print(scanner.raw_data)
        ...
        for i in range(len(scores)):
            if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0) and (labels[int(classes[i])] == detector_item_name or labels[int(classes[i])] == detect_item_name )):
                ...
                # Guide the "item" to the correct position    
                    ...
                    #############
                    # haptic right now send random integers, TODO, send correct requests
                    #############
